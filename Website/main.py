...
def edit(sno):
    if ('user' in session and session['user'] == params['admin_user']):
        if request.method == 'POST':
            ...
            if sno == '0':
                post = Posts(title=box_title, slug=box_slug, content = box_content, tagline = box_tagline, img_file = box_img_file, date = date)
                db.session.add(post)
                db.session.commit()
            else:
...
def contact():
    if (request.method == 'POST'):
        ...
        db.session.commit()
        mail.send_message('New message from ' + name, sender=email, recipients=[params['gamil-user']],
                          body=message + "\n Phone Number : " + phone)
